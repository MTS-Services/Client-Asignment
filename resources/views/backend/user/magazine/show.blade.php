<x-user::layout>
    <x-slot name="title">{{ __('Magazine Details') }}</x-slot>
    <x-slot name="breadcrumb">{{ __('Magazine Details') }}</x-slot>
                ...
                <div class="flex items-center justify-between">
                    <div class="mb-2 ps-6">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            {{ __('Published on') }} {{ $magazine->created_at_formatted }}
                        </p>
                    </div>
                    <div class="flex items-center gap-2 pe-6">
                        <x-user.primary-link href="{{ route('user.magazine-list') }}">{{ __('Back') }} <i
